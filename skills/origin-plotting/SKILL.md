...
```
[0] 提速判断（默认走快路径）
     │ 90% 场景：一次 origin_figure 完成 写数/导入→画图→验证→导出（~1s）→ 直接 [7]
     │ 只在 需要细改 / 多组对比 / 发表级精修 时才进 [1] 的完整流程
...
### 4A+ 用户给了参考图（参考图安全借鉴，P1-4）
用户发来参考 PNG/JPG 说"画成这样"时，**图形简报**流程：

1. 读参考图（模型识图），只提取**图形语法**：面板布局（几行几列）、图表家族
   （线/散点/柱/箱线）、线型符号（实/虚、圆/方）、大致配色（给 `family` 建议或
   显式取色列表）、图例位置、网格有无；
2. 把简报映射为工具参数：布局→`multi_panel` 或 `origin_manage_pages`；
   家族/线型→`plot_type`；配色→`family` 或 `style_overrides.series_colors`；
   线宽→`style_overrides.line_width_pt`；图例→`origin_edit_legend`；
3. **硬性红线**（违反=造假）：
   - **禁止从参考图像素反推/抄写数据**——数据只能来自用户的文件或用户给的数值；
   - **不复制参考图中的文字/拟合结果/物相标注/Logo/水印**；
   - 不承诺 1:1 复刻（说明"布局与风格已对齐，数据为本实验数据"）；
   - 参考图表达不了的能力（如实拒绝）不冒充支持。
