...
description: Origin 全流程 SOP（v2.2，43 工具按步骤挂载）：检查环境 → inspect 现状 → 规划（新建/编辑）→ 执行 → verify 校验（只有 fail 必须修）→ 失败恢复 → 交付。含意图→工具速查、错误码→动作映射、领域模板、细粒度改图、通道纪律。
...
# Origin SOP v2.2（决策流驱动；工具挂在步骤下，按序调用）
